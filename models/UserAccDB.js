...
class UserAccDB {
    getAllUserAcc(callback) {
        var sql = "SELECT userId, username FROM restaurantreview.user_account";
        db.query(sql, callback);
    }

    getUser(username, callback) {
        var sql = "SELECT distinct username, email_address, profile_picture from restaurantreview.user_acount WHERE username = ?";
    ...
    loginUser(username, callback) {
        var sql = "SELECT password from restaurantreview.user_account WHERE username = ?";
        db.query(sql, [username], callback);
    }
